...
def ask_copilot(
    ...
    # SYSTEM PROMPT — erweiterte Strategie & ohne HTML
    system_prompt = (
        ...
        "- RSI(14), EMA20, EMA50, MA200\n"
        "- Bollinger-Bänder (20)\n"
        "- ATR14, ADX14, Relative Volume (RVOL20)\n"
        "- Candlestick-Struktur, Swing High/Low, Marktstruktur (HH/HL vs. LH/LL)\n\n"
        "Long-only Strategie (Swing), moderates Risiko:\n"
        "- Regime-Filter: nur Long, wenn Kurs > MA200, EMA20 > EMA50 > MA200; ADX >= 22; RVOL >= 1.0; ATR% nicht extrem (>9%).\n"
        "- Setups:\n"
        "  * Trend-Dip: Trend intakt, Pullback in die Value-Zone (EMA20/EMA50-Bereich), RSI grob 40–56 steigend.\n"
        "  * Breakout-Fortsetzung: Trend intakt, Close über EMA20 und jüngstem Hoch, RSI 50–62 steigend, Volumen-Expansion (RVOL >= 1.2).\n"
        "  * Reclaim: Flush unter EMA50, dann Close > EMA50 mit RSI >= 48 steigend (RVOL >= 1.0).\n"
        "- Exits/De-Risk:\n"
        "  * Überhitzung: Close > bb_up und RSI > 72 fallend → SELL; extreme Überhitzung (RSI > 80 & deutlich über EMA20) → STRONG SELL.\n"
        "  * Trendbruch-Warnung: Close < EMA50 und RSI < 50 fallend → SELL/De-Risk.\n"
        "- Wenn Regime-Filter failt → immer HOLD (kein Long-Setup).\n\n"

...
from auto_analysis import run_auto_analysis_if_needed, log_and_notify_signal_change
...
def init_state():
    ...
    # KI / Auto-Analyse / Signal-Tracking
    st.session_state.setdefault("auto_enabled", True)
    st.session_state.setdefault("last_auto_context", None)
    st.session_state.setdefault("last_auto_reply", "")
    st.session_state.setdefault("groq_status", None)
    st.session_state.setdefault("last_signal", None)
    st.session_state.setdefault("last_signal_time", None)
...
def main():
    ...
    st.sidebar.markdown("### 🤖 Auto-Analyse")
    st.session_state.auto_enabled = st.sidebar.checkbox(
        "Auto-Analyse aktiv",
        value=st.session_state.auto_enabled,
        help="Wenn aktiv, erzeugt die KI bei neuen Daten automatisch eine kurze Markteinschätzung.",
    )
    ...
    with col_right:
        with st.container():
            ...
            # Timeframe-Buttons (horizontal, wie deine alte Version)
            ...
            try:
                ...
                if df.empty:
                    ...
                    # Zeitstempel für Signal-Tracking merken
                    st.session_state.last_signal_time = last_time.strftime("%Y-%m-%d %H:%M:%S")

                    # Signalwechsel in Verlauf loggen + E-Mail verschicken
                    log_and_notify_signal_change(
                        symbol_label,
                        tf_label,
                        sig,
                        last_price,
                    ...
                    # Auto-Analyse (Groq) nur bei geändertem Kontext
                    run_auto_analysis_if_needed(
                        df,
                        sig,
                        signal_reason,
                        symbol_label,
        ...
        # ---------------------------------------------------------
        # KI-BOT + AUTO-ANALYSE (RIGHT PANEL – BOTTOM)
        # ---------------------------------------------------------
        st.markdown("")
        with st.container():
            st.markdown('<div class="tv-card">', unsafe_allow_html=True)
            st.markdown('<div class="tv-title">KI-Assistent & Auto-Analyse</div>', unsafe_allow_html=True)

            tab_chat, tab_auto = st.tabs(["Chat", "Auto-Analyse"])
            ...
            with tab_chat:
                ...
                user_msg = st.chat_input("Frage zur aktuellen Marktsituation stellen...")
                if user_msg:
                    st.session_state.chat_history.append({"role": "user", "content": user_msg})
                    # Kompakter Kontext zur aktuellen Marktsituation
                    if df.empty or sig == "NO DATA":
                        context = "Es liegen aktuell keine verwertbaren Marktdaten vor."
                    else:
                        context = (
                            f"Aktuelle Marktdaten für {symbol_label} im Timeframe {tf_label}. "
                            f"Aktuelles Handelssignal: {sig}. Begründung: {signal_reason}. "
                            f"Letzter Schlusskurs: {last_price:.2f} USD, Änderung: {change_pct:.2f}%."
                        )
                    ...
                    messages = [
                        {
                            "role": "system",
                            "content": (
                                "Du bist ein Krypto-Trading-Assistent. "
                                "Beantworte Fragen kurz, präzise und auf Deutsch. "
                                "Gib keinen Financial Advice, sondern erläutere Chancen, Risiken und Szenarien."
                            ),
                        },
                        {
                            "role": "user",
                            "content": f"{context}\n\nFrage des Nutzers: {user_msg}",
                        },
                    ]
            ...
            # Auto-Analyse-Tab
            with tab_auto:
                if st.session_state.last_auto_reply:
                    st.markdown(st.session_state.last_auto_reply)
                else:
                    st.info(
                        "Noch keine Auto-Analyse vorhanden oder Groq nicht erreichbar. "
                        "Sobald neue Marktdaten ausgewertet werden, erscheint hier automatisch eine Einschätzung."
                    )
